...
class Model(object):
  ...
  def makeIndexNode(self, ind):
    ...
    for (label, curnode) in self.nodes.items():
      ...
      if ind.type == 'block_index':
        mapping.append(int(label))
        size_of_block = 0
        split_block = ind.symbol.split(' & ')       # Split index set by name
  ...
  def makeIndexTypedToken(self, ind):
    ...
    if ind.symbol in self.typed_tokens.keys():     # Then populate index
      for i, typedToken in enumerate(set(typed_t_species)):
        mapping.append(i)
        blocking.append(1)
    elif ind.symbol.split('_')[0] in self.typed_tokens.keys():
